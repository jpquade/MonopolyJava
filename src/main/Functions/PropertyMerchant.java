...
//    public void purchaseProperty(Player player, PropertyAttributes attributes, PropertyFinancials financials, PropertyGroupProcessor propertyGroupProcessor,
...
//            if(propertyGroupProcessor.checkForMonopoly(player, propertyAttributesMap)){
//                System.out.println();
//                System.out.println(STR."\{player.getToken()} now has a monopoly with the \{attributes.getColor()} color group which includes...");
//                System.out.println(propertyGroupProcessor.getPropertyList());
//            }
//
//        } else {
//            System.out.println();
...
//                           LinkedHashMap<String, PropertyAttributes> propertyAttributesMap,
//                           PropertyGroupProcessor propertyGroupProcessor, Scanner scanner, NumberValueCheck numberValueCheck){
//
...
//
//        if(!canAddHouse(attributes, propertyAttributesMap, propertyGroupProcessor)){
...
//
//        int colorGroupSize = propertyGroupProcessor.getPropertyList().size();
//
//        LinkedHashMap<Integer, String> listOfBuildHouse = new LinkedHashMap<>();
//        ArrayList<String> listOfUnbuildableHouse = new ArrayList<>();
//
//        int colorGroupHouseCount = propertyGroupProcessor.getColorGroupHouseCount(propertyAttributesMap);
//
//        for(int i = 0; i < colorGroupSize; i++){
//
//            String propertyName = propertyGroupProcessor.getPropertyList().get(i);
//
...
//            propertyGroupProcessor.getHouseStockOrder().put(propertyGroupProcessor.getHouseStockOrder().size() + 1 , attributes.getName());
//
...
//    }
    ...
    private boolean canAddHouse(PropertyAttributes attributes, LinkedHashMap<String, PropertyAttributes> propertyAttributesMap, PropertyGroupProcessor propertyGroupProcessor){
        ...
        if(!propertyGroupProcessor.isMonopoly() || !attributes.isImprovementAllowed()
                || propertyGroupProcessor.colorGroupHasMortgaged(propertyAttributesMap)
                || propertyGroupProcessor.colorHotelsExist(propertyAttributesMap)
                || propertyGroupProcessor.getHouseStockOrder().size() == 12) return false;

        // checks number of houses already

        int totalHouseCount = propertyGroupProcessor.getColorGroupHouseCount(propertyAttributesMap);

        if(propertyGroupProcessor.getPropertyList().size() == 2){
...
//                           LinkedHashMap<String, PropertyAttributes> propertyAttributesMap, PropertyGroupProcessor propertyGroupProcessor){
//
//        // if player has enough cash to purchase house and meets other requirements to purchase a house
//        if(financials.getPricePerImprovement() <= player.getCash() && canAddHotel(attributes, propertyAttributesMap, propertyGroupProcessor)){
...
//    }
    ...
    private boolean canAddHotel(PropertyAttributes attributes, LinkedHashMap<String, PropertyAttributes> propertyAttributesMap, PropertyGroupProcessor propertyGroupProcessor){

        int totalHouseCount = propertyGroupProcessor.getColorGroupHouseCount(propertyAttributesMap);
        int totalHotelCount = propertyGroupProcessor.getColorGroupHotelCount(propertyAttributesMap);
